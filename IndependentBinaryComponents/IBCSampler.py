...
def pw_sampler(dim, #Dimension of the state space
    ...
    while sample_index <= N:
        ...
        proposed_state = apply_gen(current_state, component)
        
        #Updating the rates and the normalization constant
        proposed_rates = update_rates(proposed_state, rates, sum_rates, component, pvals, g)
        proposed_sum_rates = proposed_rates.sum()
        
        #Calculating the acceptance probability
        ratio_states = prob_ratio(component, current_state, pvals)
...
def zanella_sampler(dim, #Dimension of the state space
    ...
    start_time = time()
    while process_time < target_time:
        ...
        jump_rates = update_rates(current_state, jump_rates, gamma, component, pvals, g)
        gamma = (jump_rates).sum()
...
def tabu_sampler(dim, #Dimension of the state space
    ...
    gamma_flip = (rates*(np.ones(dim) -alpha)).sum()
    ...
    while process_time < target_time:
        ...
        if np.random.rand(1) <= move_probability:
            ...
            #Updating the rates and the gammas
            rates = update_rates(current_state, rates, gamma_jump, component, pvals, g)
            gamma_jump = (rates*alpha).sum()
            gamma_flip = (rates*(np.ones(dim)-alpha)).sum()
